...
class App:
    def __init__(self):
        ...
        if open(self.cache_path, 'r', encoding='utf-8').read().strip() == '{}':
            self.langs_list, self.lang_ngrams_log_probs_list = self.get_langs_and_ngrams_log_probs(
    ...
    def get_langs_and_ngrams_log_probs(self, n, csv_path):
        ...
        lang_text_list = [' '.join([sentence 
                                          for sentence in train_csv[train_csv['lang'] == lang]['sentence']]) 
                          for lang in langs_list]
        lang_ngrams_list = [self.get_ngrams(lang_text_list[i], n)
                            for i in lang_range]
        return langs_list, [self.compute_log_probs(lang_ngrams_list[i]) for i in lang_range]
