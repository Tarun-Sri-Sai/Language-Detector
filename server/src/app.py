...
class App:
    ...
    def get_lang_ngrams_log_probs(self):
        ...
        lang_text_list = [
            ' '.join([sentence for sentence in self.csv_data[self.csv_data['lang'] == lang]['sentence']]) for lang in self.langs_list
        ]
        lang_ngrams_list = [
            self.get_ngrams(lang_text_list[i], self.N) for i in lang_range
        ]
        return [
            self.compute_log_probs(lang_ngrams_list[i]) for i in lang_range
        ]
